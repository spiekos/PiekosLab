...
def _annotation_score(
    ...
    if modality is not Modality.METABOLOMICS and modality is not Modality.LIPIDOMICS:
        raise ValueError("modality must be Modality.METABOLOMICS or Modality.LIPIDOMICS")
    ...
    # base: mzCloud scores, but set them to NaN where mzCloud is missing
    annotation = annotation.where(~no_mzcloud, np.nan)

    # Only compute source-based scores when at least one row has no mzCloud
    if no_mzcloud.any():
        source_scores = meta.loc[no_mzcloud].apply(_annot_source_score, axis=1)
        source_scores = source_scores.astype(float)
        source_scores = source_scores.reindex(meta.index)
        # fill NaNs (no mzCloud) with source-based scores
        annotation = annotation.fillna(source_scores)
    
    if modality == Modality.LIPIDOMICS:
        lipidid=(
                    meta.get("lipidid",
    ...
    elif modality != Modality.METABOLOMICS:
...
def _trajectory_plots(
    ...
    #binned gest age
    timepoint = final_matrix["SampleGestAge"].map(categorize_sample_time)
    for fid in selected[:6]:
        ...
        # 2) binned boxplot by timepoint bins 1–5
        valid_bin = timepoint.notna() & y.notna()
        if valid_bin.sum() < 3:
            continue

        tp_values = timepoint[valid_bin]
        y_values = y[valid_bin]
        unique_tps = sorted(pd.unique(tp_values))

        # group y-values by time bin
        data_by_tp = [y_values[tp_values == tp] for tp in unique_tps]

        fig, ax = plt.subplots(figsize=(7, 4))
        ax.boxplot(data_by_tp, tick_labels=unique_tps)
        ax.set_title(f"{analyte_names[fid]} trajectory (binned 1–5)")
        ax.set_xlabel("Gestational time bin (1–5)")
        ax.set_ylabel("Processed abundance")
        fig.tight_layout()
...
def _process_polarity(
    ...
    bad_samples = _sample_istd_mad_filter(run, sample_info, artifacts, run.modality)
    if bad_samples:
