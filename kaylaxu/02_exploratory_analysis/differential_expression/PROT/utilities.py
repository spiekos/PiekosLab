# pointing to 01 data cleaning utility file is a hardcoded file path -> need to to change with finalized file structure
"""
...
_cu_spec = _ilu.spec_from_file_location(
    ...
    "/Users/kaylaxu/Desktop/dp3_project/01_data_cleaning/PROT/utilities.py"
)
...
def run_longitudinal(
    ...
    for label, df in timepoint_dfs.items():
        if group_col in df.columns:
            df = df.loc[df[group_col].str.contains(group, na=False, regex=False)].copy() # check if group is a substring of df[group_col] -> allows for inexact string matching (ie. LO severe PE maps to PE)
...
def write_sample_count_report(
    ...
    Args:
        ...
        file_prefix:       Prefix used in plasma filenames (e.g. "PROT", "LIPD").
    """
    os.makedirs(output_dir, exist_ok=True)
    rows = []
