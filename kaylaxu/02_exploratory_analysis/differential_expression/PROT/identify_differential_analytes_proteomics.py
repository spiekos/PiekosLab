# Modify to only output significant.csv if there are signfiicant proteins?
"""
...
Longitudinal:
    ...
    step (2-1, 3-2, 4-3, 5-4). Runs per individual group (Control, FGR, HDP,
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--omics-type",
        choices=["PROT", "MTBL", "MTBL_combat", "LIPD"],
        default="PROT",
        help="Omics type for default-mode pipeline (default: PROT). "
...
def main():
    ...
    if args.mode in ("cross_sectional", "both"):
        ...
        for index, tp in enumerate(args.timepoint_files):
            df = load_data(tp)
            analyte_cols = get_analyte_columns(df)
            logger.info("Loaded %d samples × %d analytes from %s",
...
if __name__ == "__main__":
    ...
    def _run_default_mode(output_dir: str, omics_type: str = "PROT") -> None:
        wkdir = os.getcwd()
        ...
        cleaned_dir_placenta = os.path.join(
            wkdir, "data", "processed", data_subdir, "normalized_full_results"
        )
