# need to add code to enforce naming convensions, or else downstream breaks
# change so proteomics can run placenta as well as plasma
...
usage: clean_proteomics_data.py [-h] [--mode {auto,single}] [--data-dir DATA_DIR]
                                [--metadata-path METADATA_PATH]
                                [--output-dir OUTPUT_DIR] [--files FILES [FILES ...]]
                                [--meta-type {proteomics,placenta}]
                                [--output-csv OUTPUT_CSV]
options:
  -h, --help            show this help message and exit
  --mode {auto,single}  auto: discover plasma/placenta CSVs in --data-dir (default). single: run one dataset
                        with explicit --files/--meta-type/--output-csv.
  --data-dir DATA_DIR   Directory containing raw Olink CSV files (used in auto mode).
  --metadata-path METADATA_PATH
                        Path to metadata Excel file.
  --output-dir OUTPUT_DIR
                        Directory for cleaned output CSVs (used in auto mode).
  --files FILES [FILES ...]
                        Input CSV files (required in single mode).
  --meta-type {proteomics,placenta}
                        Metadata sheet type (required in single mode).
  --output-csv OUTPUT_CSV
                        Output CSV path (required in single mode).
...
# load packages for argument input and parsing
import logging
import os
import sys
import argparse
...
# -----------------------------
'''
def process_all_files(file_paths: list[str], output_csv: str, metadata_path: str, meta_type: str = "PROT"):
    """
    ...
    Args:
        ...
        meta_type: 'PROT', 'metabolomics'
    """
    run_label = "plasma" if meta_type == "PROT" else ('placenta' if meta_type == "placenta" else "none") # fix this, if you specific proteomics is only runs plasma
    ...
    if n_assay_ctrl_rows > 0:
        ...
        logger.info("[%s] removed %d rows from %d control assays (assay name contains 'control').", run_label, n_assay_ctrl_rows, n_assay_ctrl_names)
    ...
    # output missingness report for dropped assays
    if n_dropped > 0:
        logger.info(
...
def process_all_files(file_paths: list[str], output_csv: str, metadata_path: str, meta_type: str = "PROT"):
    ...
    # --- NEW: Process and save uncorrected version ---
    # Apply missingness filter to uncorrected data
    X_kept_uncorrected, _ = missingness_filter_and_group_check(
        X, groups_binary, cutoff=CUTOFF_PERCENT_MISSING, alpha_bh=0.05
    )
    
    # Imputation for uncorrected data
    X_final_uncorrected = half_min_impute_wide(X_kept_uncorrected)
    
    # Merge metadata for uncorrected data
    metadata_aligned_uncorrected = metadata.reindex(X_final_uncorrected.index)
    if meta_type == "PROT":
        subject_ids_uncorrected = X_final_uncorrected.index.to_series().str.replace(
            r"\s*[A-Z]+$", "", regex=True
        )
        metadata_aligned_uncorrected.insert(0, "SubjectID", subject_ids_uncorrected.values)
    final_output_uncorrected = pd.concat([metadata_aligned_uncorrected, X_final_uncorrected], axis=1)
    
    # Save uncorrected version
    uncorrected_out_path = f"/Users/kaylaxu/Desktop/dp3_project/data/raw/extracted/PROT/uncorrected_{meta_type}_expr.csv"
    final_output_uncorrected.to_csv(uncorrected_out_path, index=True)
    logger.info("Saved uncorrected expression data to: %s", uncorrected_out_path)
    # -------------------------------------------------
...
if __name__ == "__main__":
    ...
    def _run_default_mode() -> None:
        ...
        if plasma_files:
            ...
            process_all_files(plasma_files, out_csv_plasma, metadata_path, meta_type="PROT")
