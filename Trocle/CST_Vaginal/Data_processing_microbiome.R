...
# Optional sanity check: negative controls vs study samples
...
# Taxa present at >= 1% in both negative controls and study samples
...
filter_read_depth <- function(cohort_data, cohort_name) {
  ...
  p_before <- ggplot(depth_df, aes(x = Total_Reads)) +
    ...
    labs(title = paste(cohort_name, "- Read Depth Distribution"),
...
filter_features <- function(cohort_data, cohort_name) {
  ...
  # 4a. Minimum Count Threshold (Per-Sample Detection) 
  presence_mat <- count_mat >= 3
  feat_after_count <- sum(rowSums(presence_mat) > 0)
  
  # 4b. Prevalence Filter 
  prevalence <- rowSums(presence_mat) / total_samples
  prev_keep <- prevalence >= 0.10
  feat_after_prev <- sum(prev_keep)
  
  # 4c. Mean Relative Abundance Filter
  sample_totals <- colSums(count_mat)
  ra_mat <- sweep(count_mat, 2, sample_totals, "/")
  
  # Mean RA per feature and RA threshold: >= 0.005
  mean_ra <- rowMeans(ra_mat, na.rm = TRUE)
  ra_keep <- mean_ra >= 0.005
  feat_after_ra <- sum(prev_keep & ra_keep)
  
  # Combine filters

  final_keep <- prev_keep & ra_keep
  ...
  message(sprintf("Features passing Prevalence & Mean RA (>= 0.005): %d", feat_after_ra))
  
  return(filtered_data)
}

# Apply filters independently per cohort
crib_filtered_features <- filter_features(crib_genus, "CRIB")
spec_filtered_features <- filter_features(spec_genus, "SPEC")

## Step 5:
convert_to_ra <- function(filtered_data, cohort_name) {
  
  # Sample columns and count matrix
  meta_cols <- c("#OTU ID", "Kingdom", "Phylum", "Class", "Order", 
                 "Family", "Genus", "Species", "Consensus Lineage")
  sample_cols <- setdiff(colnames(filtered_data), meta_cols)
  count_mat <- as.matrix(filtered_data[, sample_cols])
  
  # Recalculate per-sample totals 
  sample_totals <- colSums(count_mat)
  
  # Divide each feature count 
  ra_mat <- sweep(count_mat, 2, sample_totals, "/")
  
  # Verify column sums
  col_sums_check <- colSums(ra_mat)
  all_sum_to_one <- all(abs(col_sums_check - 1.0) < 1e-6)
  
  if (all_sum_to_one) {
    message(sprintf("[%s] All sample columns  sum to 1.0.", cohort_name))
  } else {
    warning(sprintf("[%s] Not all sample columns sum to 1.0.", cohort_name))
  }
  
  # Recombine the matrix
  ra_data <- filtered_data
  ra_data[, sample_cols] <- ra_mat
  
  return(ra_data)
}

# Apply conversion independently per cohort
crib_ra <- convert_to_ra(crib_filtered_features, "CRIB")
spec_ra <- convert_to_ra(spec_filtered_features, "SPEC")
...
transform_clr <- function(count_data, cohort_name) {
  ...
  # 6a. Zero Imputation 
  imputed_counts_t <- cmultRepl(count_mat_t, method = "CZM", output = "p-counts", suppress.print = TRUE)
