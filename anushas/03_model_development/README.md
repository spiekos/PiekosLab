...
This directory contains the modeling workflow for Fitbit-derived predictors and clinical and demographic variable outcomes.

## Contents

- `run_fitbit_glm_interactions.py` — runs a generalized linear modeling pipeline that tests Fitbit metrics against clinical/demographic outcomes, adjusting for clinical covariates and interaction terms.

## Workflow

### `run_fitbit_glm_interactions.py`

* **Purpose:** Fit GLMs for Fitbit activity/sleep/heart rate metrics with clinical/demographic outcomes.
* **Inputs:**
...
* **Modeling strategy:**
  ...
  * Fits separate GLMs for each Fitbit metric and each clinical/demographic outcome.
...
- The script currently uses a hard-coded set of Fitbit metrics and clinical/demographic outcomes.
